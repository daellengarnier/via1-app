...
// WG-Passwort-Verwaltung als Sektion fuer /admin/users.
// Liste aller 6 WGs mit "Setzen"/"Aendern"/✕ pro WG.
export function WgPasswordsSection() {
  ...
  if (error) {
    return (
      <section className="mb-6 rounded-lg border border-red-500/30 bg-red-500/5 p-3">
        <p className="text-sm text-red-300">{error}</p>
      </section>
    );
  }

  const withPw = wgs.filter((w) => w.hasPassword).length;
  const without = wgs.length - withPw;

  return (
    <section className="mb-6 rounded-lg border border-secondary/30 bg-secondary/5">
      <button
        onClick={() => setExpanded((v) => !v)}
        className="flex w-full items-center justify-between p-3 text-left"
      >
        <span className="font-display text-[11px] font-bold uppercase tracking-widest text-secondary">
          🔐 WG-Passwoerter
        </span>
        <span className="font-mono text-[10px] text-secondary/70">
          {loading
            ? "…"
            : `${withPw}/${wgs.length} gesetzt${without ? ` · ${without} offen` : ""}`}{" "}
          {expanded ? "▾" : "▸"}
        </span>
      </button>

      {expanded && (
        <div className="space-y-2 border-t border-secondary/20 p-3">
          {loading && <p className="text-xs text-gray-500">Lade...</p>}
          {wgs.map((w) => {
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {w.hasPassword ? (
                      <p className="mt-1 text-[10px] text-emerald-400">
                        ...
                        {w.passwordSetAt
                          ? ` · ${new Date(w.passwordSetAt).toLocaleDateString(
                              "de-CH"
                            )}`
                          : ""}
                      </p>
